...
(function rd_CompSetter(thisObj)
{
	...
	rd_CompSetterData.scriptTitle = rd_CompSetterData.scriptName + " v3.2";
	...
	function rd_CompSetter_buildUI(thisObj)
	{
		...
		if (pal !== null)
		{
			...
			pal.grp.renderer.lst.selection = pal.grp.preserveFR.lst.selection = pal.grp.preserveRes.lst.selection = 0;
	...
	function rd_CompSetter_compSetRecursively(comp, width, height, par, fps, dur, ren, fr, res, sa, sp, spf, asl, recurse)
	{
		var layer;
		...
		for (var i=1; i<=comp.numLayers; i++)
		{
			...
			if (recurse && (layer instanceof AVLayer) && (layer.source !== null) && (layer.source instanceof CompItem))
				rd_CompSetter_compSetRecursively(layer.source, width, height, par, fps, dur, ren, fr, res, sa, sp, spf, asl, recurse);
	...
	function rd_CompSetter_doCompSetter()
	{
		...
		for (var i=0; i<selComps.length; i++)
		{
			...
			var newRenderer = this.parent.parent.renderer.lst.selection.index - 1;
			
			var newPreserveFR = this.parent.parent.preserveFR.lst.selection.index - 1;
			var newPreserveRes = this.parent.parent.preserveRes.lst.selection.index - 1;
			
			var newSA = this.parent.parent.sa.opt.value ? parseFloat(this.parent.parent.sa.fld.text) : -1;
			var newSP = this.parent.parent.sp.opt.value ? parseFloat(this.parent.parent.sp.fld.text) : -1;
			var newSPF = this.parent.parent.spf.opt.value ? parseFloat(this.parent.parent.spf.fld.text) : -1;
			var newASL = this.parent.parent.asl.opt.value ? parseFloat(this.parent.parent.asl.fld.text) : -1;
			
			rd_CompSetter_compSetRecursively(comp, newWidth, newHeight, newPAR, newFPS, newDur, newRenderer, newPreserveFR, newPreserveRes, newSA, newSP, newSPF, newASL, this.parent.parent.recursive.value);
		}
